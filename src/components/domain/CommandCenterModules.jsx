...
export function ProcessRiskBoard({ rows = [] }) {
  return (
    <Panel title="Process bottleneck and handoff board">
      <ul className="row-list">
        {rows.map((row) => (
          <li key={row.id} className={`impact-cell tone-${toSeverityTone(row.severity)}`.trim()}>
            <div className="process-risk-row">
              <div>
                <strong>{row.label}</strong>
                <div className="meta">{row.lane}</div>
              </div>
              <span className="chip">{row.severity}</span>
            </div>
            <div className="meta">Risk rationale: {row.rationale}</div>
          </li>
        ))}
      </ul>
    </Panel>
...
export function EventSequenceBoard({ rows = [] }) {
  return (
    <Panel title="Event sequence diagnostics">
      <div className="impact-matrix">
        {rows.map((row) => (
          <article key={row.track} className={`impact-cell tone-${toSeverityTone(row.severity)}`.trim()}>
            <header>
              <strong>{row.track}</strong>
              <span className="chip">{row.severity}</span>
            </header>
            <div className="meta">Events in sequence: {row.count}</div>
            <div className="meta">Anomalies: {row.anomalies}</div>
            <div className="meta">KPI-correlated: {row.correlated}</div>
          </article>
        ))}
      </div>
    </Panel>
